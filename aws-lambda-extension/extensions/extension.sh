#!/bin/bash
# Based on https://github.com/aws-samples/aws-lambda-extensions/blob/main/custom-runtime-extension-demo/extensionssrc/extensions/extension1.sh
...
SORAPROXY_ADDRESS="0.0.0.0"
SORAPROXY_PORT="8080"

# Graceful Shutdown
_term() {
  echo "[${LAMBDA_EXTENSION_NAME}] Received SIGTERM"
  # forward SIGTERM to child processes and exit
...
do
  ...
  /opt/bin/curl -sS -L -XGET "http://${AWS_LAMBDA_RUNTIME_API}/2020-01-01/extension/event/next" --header "Lambda-Extension-Identifier: ${EXTENSION_ID}" > "$TMPFILE" &
  PID=$!
  forward_sigterm_and_wait
